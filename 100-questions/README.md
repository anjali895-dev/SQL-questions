...
- [Q.13](#Q.13) Show products in the 'Electronics' category from Dim_Products.
- [Q.14](#Q.14) List payments made via 'UPI' from Fact_Payments.
- [Q.15](#Q.15) Find all orders with TotalOrderAmount greater than 1000 from Fact_Orders.
- [Q.16](#Q.16) Show customers from the city 'Bengaluru' in Dim_Customers.
- [Q.17](#Q.17) Find inventory movements of type 'IN' from Fact_InventoryMovement.
- [Q.18](#Q.18) List orders where DiscountAmount is greater than 0.
- [Q.19](#Q.19) List all customers sorted by CustomerName in ascending order.
- [Q.20](#Q.20) Show orders sorted by TotalOrderAmount descending.
- [Q.21](#Q.21) List products sorted by Category then by ProductName.
- [Q.22](#Q.22) Show the top 5 most recent orders from Fact_Orders.
- [Q.23](#Q.23) Retrieve the first 10 customers who signed up (earliest SignupDate).
- [Q.24](#Q.24) Show 10 order items starting from the 11th record (use OFFSET).
- [Q.25](#Q.25) Get the top 3 highest paid orders by PaidAmount from Fact_Payments.
- [Q.26](#Q.26) Find customers who signed up after '2023-01-01' and belong to the 'Premium' segment.
- [Q.27](#Q.27) List orders placed in January 2024 from Fact_Orders.
- [Q.28](#Q.28) Show products whose ProductName contains the word 'Pro'.
- [Q.29](#Q.29) Find all cart events where EventType is either 'AddToCart' or 'Purchase'.
- [Q.30](#Q.30) Find orders where DeliveryFee is NULL from Fact_Orders.
- [Q.31](#Q.31) Count the number of customers in each CustomerSegment.
- [Q.32](#Q.32) Find the total TotalOrderAmount for each OrderStatus in Fact_Orders.
- [Q.33](#Q.33) Count how many products exist in each Category from Dim_Products.
- [Q.34](#Q.34) Find the average UnitPrice per ProductID in Fact_OrderItems.
- [Q.35](#Q.35) Count the number of payments per PaymentMethod in Fact_Payments.
- [Q.36](#Q.36) Find total Quantity moved per ProductID in Fact_InventoryMovement.
- [Q.37](#Q.37) Count cart events by EventType from Fact_CartEvents.
- [Q.38](#Q.38) Find the maximum TotalAmount per OrderID in Fact_OrderItems.
- [Q.39](#Q.39) Show CustomerSegments that have more than 2 customers.
- [Q.40](#Q.40) List ProductIDs where total ordered quantity (from Fact_OrderItems) exceeds 4.
- [Q.41](#Q.41) Find PaymentMethods where total PaidAmount is more than 2500.
- [Q.42](#Q.42) Show OrderIDs in Fact_OrderItems that have more than 2 line items.
- [Q.43](#Q.43) Join Fact_Orders with Dim_Customers to show CustomerName, OrderID, and TotalOrderAmount.
- [Q.44](#Q.44) Join Fact_OrderItems with Dim_Products to show ProductName and Quantity ordered.
- [Q.45](#Q.45) Join Fact_Orders with Dim_Location to show OrderID and Zone.
- [Q.46](#Q.46) Join Fact_Payments with Fact_Orders to show OrderDate and PaidAmount.
- [Q.47](#Q.47) Join Fact_DeliveryLogs with Fact_Orders to show CustomerID and DeliveryPartner.
- [Q.48](#Q.48) Join Fact_CartEvents with Dim_Products to show ProductName and EventType.
...
- [Q.56](#Q.56) For each customer, show their total number of orders and total spend. Join Dim_Customers with Fact_Orders.
- [Q.57](#Q.57) Show the top 5 products by total revenue. Join Dim_Products with Fact_OrderItems.
- [Q.58](#Q.58) List customers who have never placed an order (use LEFT JOIN and check for NULL).
- [Q.59](#Q.59) Show each order with customer name, zone, and delivery partner. Requires 3 table joins.
- [Q.60](#Q.60) Join Fact_Orders, Fact_OrderItems, and Dim_Products to show each order's product names and quantities.
- [Q.61](#Q.61) Find customers whose total spend across all orders exceeds 500.
- [Q.62](#Q.62) Find product categories where average UnitPrice in Fact_OrderItems is above 20.
- [Q.63](#Q.63) Show zones that have more than 2 orders placed. Join Fact_Orders with Dim_Location.
- [Q.64](#Q.64) Find total discount given per CustomerSegment. Join Fact_Orders with Dim_Customers.
- [Q.65](#Q.65) Show total inventory IN and OUT per ProductID from Fact_InventoryMovement.
- [Q.66](#Q.66) Find average order value per State by joining Fact_Orders with Dim_Customers.
- [Q.67](#Q.67) For each product, show the count of 'AddToCart' events vs 'Purchase' events from Fact_CartEvents. Join with Dim_Products.
- [Q.68](#Q.68) Find orders placed in Q4 (October–December) of 2024 with status 'Delivered'.
- [Q.69](#Q.69) Show customers who placed orders in more than one different zone. Join Fact_Orders with Dim_Location and Dim_Customers.
- [Q.70](#Q.70) List orders along with their payment status. Show OrderID, OrderStatus, and PaymentStatus.
- [Q.71](#Q.71) Find the month with the highest total revenue from Fact_Orders.
- [Q.72](#Q.72) Show the full order details including product names, customer name, and payment method for OrderID = 1001.
- [Q.73](#Q.73) Find SubCategories where total quantity sold (Fact_OrderItems) is less than 20.
- [Q.74](#Q.74) List products that appear in Fact_CartEvents but have never been ordered in Fact_OrderItems.
- [Q.75](#Q.75) Show total revenue per Category per Month. Join Fact_OrderItems with Dim_Products and Fact_Orders.
- [Q.76](#Q.76) Show DeliveryPartners who have handled more than 2 orders.
- [Q.77](#Q.77) Show each customer's last order date and amount. Join Dim_Customers with Fact_Orders.
- [Q.78](#Q.78) Find customers who signed up in the year 2022 and are from the 'South' zone. Join Dim_Customers with Dim_Location.
- [Q.79](#Q.79) Find average delivery fee per Zone by joining Fact_Orders with Dim_Location.
- [Q.80](#Q.80) Find the top 3 customers by number of distinct products ordered. Join Dim_Customers, Fact_Orders, Fact_OrderItems.
- [Q.81](#Q.81) Show customers who have placed more than 5 orders with OrderStatus = 'Cancelled'. 
- [Q.82](#Q.82) Show the net inventory (IN minus OUT) per product. Join Fact_InventoryMovement with Dim_Products. 
- [Q.83](#Q.83) List all 'Failed' payments where the corresponding order has status 'Delivered'. 
- [Q.84](#Q.84) Count the number of unique customers per Zone who placed at least one order. 
- [Q.85](#Q.85) Show Zones where total revenue exceeds 200. Join Fact_Orders with Dim_Location. 
- [Q.86](#Q.86) Show total quantity of each product sold per Quarter. Use Dim_Date joined via OrderDate. 
- [Q.87](#Q.87) Find the payment method with the highest total PaidAmount per CustomerSegment.
- [Q.88](#Q.88) Show cart events that happened on weekends. Join Fact_CartEvents with Dim_Date.
- [Q.89](#Q.89) List products that are in the cart but never purchased. Use Fact_CartEvents grouping by EventType.
- [Q.90](#Q.90) Find Pincodes where the number of delivered orders is less than 5.
- [Q.91](#Q.91) Show orders placed on weekdays vs weekends (count each). Join Fact_Orders with Dim_Date.
- [Q.92](#Q.92) Calculate the cart-to-purchase conversion rate per product (purchases / total cart events). Use Fact_CartEvents joined with Dim_Products.
- [Q.93](#Q.93) Show products where inventory quantity received (MovementType = 'IN') is less than quantity dispatched ('OUT'). 
- [Q.94](#Q.94) Show each customer's most recently ordered product. Join Dim_Customers, Fact_Orders, Fact_OrderItems, Dim_Products.
- [Q.95](#Q.95) Find the top 3 zones by average order value. Join Fact_Orders with Dim_Location.
- [Q.96](#Q.96) Find customer segments where the average discount given is more than 10% of average order value.
- [Q.97](#Q.97) Show all orders where payment was successful but delivery status is still 'Pending'. Join Fact_Payments and Fact_DeliveryLogs.
- [Q.98](#Q.98) Find the top 5 customers by number of unique product categories purchased.
- [Q.99](#Q.99) Find months where total cancelled order value exceeds 500.
- [Q.100](#Q.100) Build a full customer order summary: customer name, segment, total orders, total spend, total discount, and average order value. Use all relevant tables.
